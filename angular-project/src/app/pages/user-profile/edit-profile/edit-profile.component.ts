...
import { validationMessages } from './edit-profile.validation-messages';
...
export class EditProfileComponent implements OnInit {
  ...
  imgHost = `${environment.backendHost}/images/`
  ...
  onFileChanged(event: any) {
    ...
    this._userService.updateUserProfileImage(this.currentUser.userDetailsId, formData).subscribe((res: any) => {
      if (res && res.email) {
        this._toastr.success("profile picture updated successfully");
        this._userDetailsManager.settUserDetails(res);
        setTimeout(() => window.location.reload(), 2000)
      } else {
        this._toastr.error("Error occurred while updating profile picture")
      }

    }, err => this._toastr.error("Error occurred while updating profile picture"))
  }
